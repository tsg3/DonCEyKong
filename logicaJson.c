...
void crearTexto(char* SendBuff){
    ...
    if(strcmp(barril, "1") == 0){
        strcat(SendBuff,"barril1");
    }
    if(strcmp(barril, "2") == 0){
        strcat(SendBuff,"barril2");
    }
    if(strcmp(barril, "3") == 0){
        strcat(SendBuff,"barril3");
    }

    strcat(SendBuff, "\", \"marioX\": \"");

    itoa(posMarioX,transform,10);

    strcat(SendBuff, transform);
    strcat(SendBuff, "\", \"marioY\": \"");

    itoa(posMarioY,transform,10);

    strcat(SendBuff, transform);
    strcat(SendBuff, "\", \"fuegos\": \"fueguito\", \"vidas\": \"");
